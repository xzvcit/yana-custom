...
class AMI extends EventEmitter {
  ...
  _connect() {
    ...
    this.socket.on("end", () =>
      this._socketError(new Error("Connection closed"))
    );
    this.socket.connect(
  ...
  _onFirstLine(line) {
    ...
    if (!version) {
      this.socket.destroy(
        new Error("Connection Error: server replied with unknown signature")
      );
      return;
    }
  ...
  _processMessage(rawMsg) {
    ...
    if (objMsg.event) {
      ...
      if (objMsg.event === "DTMFBegin") {
        ...
        this.emit("DTMF", newEvent);
      }
      if (objMsg.event === "Hangup") {
        this.emit("Hangup", objMsg);
      }
      if (objMsg.event === "DialBegin") {
        this.emit("DialBegin", objMsg);
      }
    ...
    if (action) {
      if (
        objMsg.response === "Success" &&
        (objMsg.eventlist === "start" ||
          (this._eventListHack && REGEX_WILL_FOLLOW.test(objMsg.message)))
      ) {
        objMsg.eventlist = [];
        ...
        if (
          objMsg.eventlist ||
          (this._eventListHack && REGEX_COMPLETE.test(objMsg.event))
        ) {
